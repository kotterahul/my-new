...
def login():
    ...
    now = datetime.now(tz=indian_timezone).strftime('%Y-%m-%d %H:%M:%S %Z%z')
    ...
    for collection_name in collections:
        ...
        if user and bcrypt.check_password_hash(user['password'], password):
            ...
            correct_user_data = {'username': username,'useragent': f'Logged in through {get_user_agent_info(user_agent)}','login_time':now}
    ...
    incorrect_user = incorrect_users.find_one({'ip_address': request.remote_addr})
    if incorrect_user and 'attempts' in incorrect_user and len(incorrect_user['attempts']) >= 3 and now - incorrect_user['attempts'][-1]['attempt_time'] <= timedelta(seconds=30):
    ...
    new_attempt = {'username': username ,'useragent': f'Tried to log in using {user_agent1}','attempt_time': now}
